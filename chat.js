const ChatState = {
    ...
    // ✅ v2.5: track all listeners for cleanup
    listeners: {},
...
// ==============================================
// Sanitizers (CSS injection prevention)
// ==============================================
...
// ==============================================
// ✅ v2.5: cleanup — يُستدعى عند logout/switchRoom
// ==============================================
...
function initChat(){
    ...
    buildRoomsList();buildBackgroundsList();
...
function switchRoom(roomId,roomTitle){
    ...
    ChatState.seenMessages.clear(); // ✅ v2.5: clear old room messages
...
function applyFrameToWrapper(wrapper,frameId){
    ...
    wrapper.querySelectorAll('.dynamic-frame-wrapper,.qcf,.avatar-frame,.qamar-frame').forEach(el=>el.remove());
    wrapper.style.position='relative';
    if(typeof CSS_FRAMES_DATA!=='undefined'&&CSS_FRAMES_DATA[frameId]&&typeof applyCSSFrame==='function'){
        try{applyCSSFrame(wrapper,frameId);return}catch(e){console.warn('CSS frame error:',e)}
    }
    if(typeof FRAMES_DATA!=='undefined'&&FRAMES_DATA[frameId]){
        const f=FRAMES_DATA[frameId];
        if(f.url){
            const img=document.createElement('img');img.src=f.url;img.className='dynamic-frame-wrapper';
            img.style.cssText='position:absolute;top:-6px;left:-6px;width:calc(100% + 12px);height:calc(100% + 12px);pointer-events:none;z-index:25;object-fit:contain;';
            wrapper.appendChild(img);return;
        }
    }
    if(Array.isArray(FRAMES)&&typeof FRAMES.find==='function'){
        const fr=FRAMES.find(f=>f.id===frameId);
        if(fr){
            const el=document.createElement('div');el.className='dynamic-frame-wrapper';el.style.cssText=fr.style;
...
// ✅ v2.5: rate limit على البحث بالكود
async function openProfileByCode(code){
    if(!code)return;
    const now = Date.now();
    if (now - ChatState._lastCodeLookup < 3000) {
        showToast('fa-clock','⏳ انتظر قليلًا قبل البحث مجددًا');
        return;
    }
...
function displayMessage(msg,msgId){
    ...
    if(msg.senderUid===(user?user.uid:'')&&user&&user.avatar)url=user.avatar;
    ...
    ai.onclick=()=>openUserProfile(msg.senderUid,msg.senderName);
    aw.appendChild(ai);
...
// ✅ v2.5: sanitized colors/gradients against CSS injection
...
// ✅ v2.5: مفتاح موحّد + إزالة king_archive (تسريب خصوصية)
async function sendPrivateMsg(){
    ...
    // ✅ v2.5: unified key on both sides
...
// ✅ v2.5: استخدام .collapsed بدل .hidden لتفادي تعارض CSS
...
function closeAllPanels(){
    ['rooms-sidebar','settings-sidebar','notif-sidebar','pm-sidebar'].forEach(id=>{const e=document.getElementById(id);if(e)e.classList.remove('open')});
...
function refreshAvatarsInMessages(newFrame){
    ...
    document.querySelectorAll('[data-sender-uid="'+u.uid+'"] .message-avatar-wrapper').forEach(w=>{
        w.querySelectorAll('.dynamic-frame-wrapper,.qcf,.avatar-frame,.qamar-frame').forEach(e=>e.remove());
...
// ✅ v2.5: selective listening — skip lastSeen-only updates
...
console.log('✅ chat.js v2.5 loaded — sanitized + cleanup + fixed keys 🔒');